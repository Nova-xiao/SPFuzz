...
nod_alloc_procinfo(void)
{
    ...
    // // AFL-SYS eliminate kernel buffer cost
...
{
    // // AFL-SYS copy buffer
    // // reset_buffer(&global_buffer, NOD_INIT_INFO);
    // memcpy(global_buffer.info, p->buffer.info, sizeof(struct nod_buffer_info));
    // memcpy(global_buffer.buffer, p->buffer.buffer, p->buffer.info->tail);
    // global_buffer.event_count = p->buffer.event_count;
