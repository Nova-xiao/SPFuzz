...
// AFL-SYS global buffer
struct nod_kbuffer global_buffer;
...
nod_dev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    case NOD_IOCTL_READ_GLOBAL_BUFFER_COUNT_INFO:
        ...
        cinfo.unflushed_count = global_buffer.info->nevents;
        cinfo.unflushed_len = global_buffer.info->tail;
    ...
    case NOD_IOCTL_FETCH_GLOBAL_BUFFER:
        ...
        fetch.len = global_buffer.info->tail;
        copy_to_user(fetch.buf, global_buffer.buffer, fetch.len);
