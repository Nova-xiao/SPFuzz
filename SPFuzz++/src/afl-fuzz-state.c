...
void afl_states_stop(void) {
  ...
  LIST_FOREACH_NEW(&afl_states, afl_state_t, {
    ...
    // AFL-SYS MODIFICATION FOR DEBUGGING

      LOGWRITE_ALWAYS("killing child_pid %d with signal %d\n",
                      el->fsrv.child_pid, el->fsrv.child_kill_signal);
      kill(el->fsrv.child_pid, el->fsrv.child_kill_signal);
    if (el->fsrv.fsrv_pid > 0) {
      LOGWRITE_ALWAYS("killing fsrv_pid %d with signal %d\n",
                      el->fsrv.fsrv_pid, el->fsrv.fsrv_kill_signal);
...
void afl_states_clear_screen(void) {
  ...
  LIST_FOREACH_NEW(&afl_states, afl_state_t, { el->clear_screen = 1; });

}

void afl_states_request_skip(void) {

  LIST_FOREACH_NEW(&afl_states, afl_state_t, { el->skip_requested = 1; });
