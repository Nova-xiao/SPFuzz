...
void afl_fsrv_start(afl_forkserver_t *fsrv, char **argv,
  ...
  // AFL-SYS NOTE: where the forkserver is started
  ...
  if (!fsrv->fsrv_pid) {
    ...
    // AFL-SYS NOTE: The entry of the forkserver logic
    // Refer to fsrv_exec_child for the actual execv
    // fsrv->init_child_func(fsrv, argv);

    // AFL-SYS MODIFICATION
    afl_sys_setup();
    afl_sys_forkserver(fsrv->target_path, argv);
...
afl_fsrv_run_target(afl_forkserver_t *fsrv, u32 timeout,
  ...
  // ACTF("Forkserver got new process: %d", fsrv->child_pid);
...
void afl_fsrv_killall() {
  ...
  LIST_FOREACH_NEW(&fsrv_list, afl_forkserver_t, {
